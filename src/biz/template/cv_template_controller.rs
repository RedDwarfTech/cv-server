use crate::service::template::template_service::{get_online_tempalte_list, get_tempalte_by_id};
...
pub fn get_list(ids: Option<String>) -> content::RawJson<String> {
    if let Some(ids) = ids {
        ...
        let numbers: Vec<i64> = id_arr
            ...
            .collect();
        let gen_cv = get_online_tempalte_list(Some(numbers));
        return box_rest_response(gen_cv);
    } else {
        let gen_cv = get_online_tempalte_list(None);
        return box_rest_response(gen_cv);
    }
}
