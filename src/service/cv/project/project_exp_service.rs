...
pub fn add_project(request: &Json<WorkRequest>, login_user_info: &LoginUserInfo) -> Result<CvWorkExp, diesel::result::Error> {
...
pub fn get_ui_project_list(cv_id: &i64, login_user_info: &LoginUserInfo) -> Vec<CvWorkExp> {
...
pub fn _get_project_list(cv_id: &i64) -> Vec<CvWorkExp> {
    use crate::model::diesel::cv::cv_schema::cv_work_exp as cv_work_table;
    let mut query = cv_work_table::table.into_boxed::<diesel::pg::Pg>();
    query = query.filter(cv_work_table::cv_id.eq(cv_id));
    let cvs = query
