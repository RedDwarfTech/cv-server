...
pub fn get_ui_lang_list(cv_id: &i64, login_user_info: &LoginUserInfo) -> Vec<CvLang> {
    ...
    let cvs = query
        .load::<CvLang>(&mut get_connection())
...
pub fn del_lang_item(item_id: &i64, login_user_info: &LoginUserInfo) -> bool {
    use crate::model::diesel::cv::cv_schema::cv_lang::dsl::*;
    let predicate = crate::model::diesel::cv::cv_schema::cv_lang::id
        .eq(item_id)
        .and(crate::model::diesel::cv::cv_schema::cv_lang::user_id.eq(login_user_info.userId));
    let delete_result = diesel::delete(cv_lang.filter(predicate)).execute(&mut get_connection());
    match delete_result {
        Ok(_v) => {
            return true;
        }
